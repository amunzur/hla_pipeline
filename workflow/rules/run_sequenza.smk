...
rule run_sequenza_utils:
    ...
    shell:
        ...
        for CHROM in _chr{{1..22}}.seqz
        do
            touch $DIR_results/{params.NAME_tumor}/{params.NAME_tumor}$CHROM
            echo sequenza-utils bam2seqz --fasta {input.PATH_hg38} -n $DIR_bams_split$NAME_WBC$CHROM -t $DIR_bams_split/{params.NAME_tumor}$CHROM -gc {input.PATH_gcwig} --het 0.4 -N 40 |  grep -v ^K | grep -v ^G | grep -v ^M | (sed -u 1q; sort -k1,1V -k2,2n) > $DIR_results/{params.NAME_tumor}/{params.NAME_tumor}$CHROM
        done
        """
